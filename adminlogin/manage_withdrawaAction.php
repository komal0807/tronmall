...
if (isset($_POST['type'])) {
	...
	if ($_POST['type'] == 'accept') {
        /* fetch bal */
        	$useridQuery = mysqli_query($con, "select`amount` from `tbl_withdrawal` where `id`='" . $id . "' and status=1");
		$userResult = mysqli_fetch_array($useridQuery);
		$amount = $userResult['amount']/100*5;
		
		$sqlA = mysqli_query($con, "Update `tbl_withdrawal` set status = '0', amount = '".$amount."',createdate = '" . $today . "' where `id`='" . $id . "' ");
