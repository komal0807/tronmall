  ...
  /* 
    ...
    */
    ...
    // print_r($_POST); die;
        ...
        if ($chkorderRow === 0) {
          ...
          if (mysqli_num_rows($reswall) > 0) {
            ...
            if($walletbalance == 0)
            {
               ...
               // 28-10-2022
              $userData = mysqli_query($con,"select * from `tbl_user` where `id`='".$userid."'");
              $getShareUserCode=mysqli_fetch_assoc($userData);
              $chkrcode=mysqli_query($con,"select * from `tbl_user` where `owncode`='".$getShareUserCode['code']."'");
              ...
              $useridshare=$codeRowdata['id'];
                ...
                $codeRow=mysqli_num_rows($reswall11);
                $rowwall12 = mysqli_fetch_assoc($reswall11);

                $walletbalan= $rowwall12['amount']+118;
                if($codeRow > 0)
                {

                    $sqlshare= mysqli_query($con, "INSERT INTO `tbl_rechargetemp`(`userid`,`amount`)VALUES('".$useridshare."','118')");
                        
                    
                }else{
                  
                    $sqlshare= mysqli_query($con,"UPDATE `tbl_rechargetemp` SET `amount` = '".$walletbalan."' WHERE `userid`= '".$useridshare."'");
                ...
                $sql65= mysqli_query($con,"INSERT INTO `tbl_bonus`(`userid`,`amount`,`level1`,`level2`,`level3`) VALUES ('".$useridshare."','118','118','0','0')");
                  ...
                  // update bonus amount in main wallet balane 22-11-22
                  $sql = "SELECT * FROM `tbl_wallet` WHERE `userid`='$useridshare'";
                  $result = mysqli_query($con, $sql);
                  $row = mysqli_fetch_assoc($result);
                  $walletbalance = $row['amount'];
                  $finalbalanceCredit = $walletbalance + 118;
                  $sqlwallet = mysqli_query($con, "UPDATE `tbl_wallet` SET `amount` = '$finalbalanceCredit' WHERE `userid`= '$useridshare'");
        ...
        $userid = $_POST['userid'];
        $check_for_first_recharge = mysqli_query($con, "select COUNT(*) as num from `tbl_recharge` where `userid`='" . $userid . "' AND status = 1 ");
        $recharge_row = mysqli_fetch_assoc($check_for_first_recharge);
        
        if ($recharge_row['num'] == '0') {
          recharge_bonus($con, $userid, $amount);
        }
        
        $wal1 = mysqli_query($con, "UPDATE `tbl_recharge` SET status = 1 WHERE userid =  '$userid'");
...
<head>
  ...
  <title>DHB Royal | Manage Deposit</title>
    ...
    <div class="content-wrapper">
      ...
      <section class="content-header">
        ...
        <ol class="breadcrumb">
          ...
          <li class="active">Manage Deposit</li>
        </ol>
      </section>
